...
import com.pl.prod.service.IpTokenService;
...
public class GetIpTokenModelImpl implements IIpTokenContract.Model {
    private static final String TAG = "ILoginModel";
    private int count = 0;

    @Override
    public void getIpToken(String deviceId, final ICallback callback) throws Exception {
        if (NetWorkUtils.isConnected(PlApplication.app)) {
            getIpMsg(deviceId, callback, IpServerUrls.GET_IP_TOKEN);
        } else {
    ...
    private void getIpMsg(final String deviceId, final ICallback callback, String url) {
        ...
        final IpTokenService request = retrofit.create(IpTokenService.class);
        request.getToken()//获取Observable对象
                ...
                .subscribe(new Subscriber<ResponseBody>() {
                    ...
                    public void onError(Throwable e) {
                        e.printStackTrace();
                        LogUtil.err(TAG, e.getMessage());
                        callback.setFailure("token 获取失败");
                    }
