...
//                    socket.setSoTimeout ( 2 * 1000 );//设置超时时间
                    if (isConnected()) {
                        TaskCenter.ipAddress = ipAddress;
                        TaskCenter.port = port;
...
//                System.arraycopy(bt, 0, bs, 0, length);
                    ...
                    byte strRxBuf[] = new byte[length];
                    int len = socketReader.read(strRxBuf, 0, length);
                    StringBuffer sb = new StringBuffer();

                    if (len > 0) {
                        for (int i = 0; i < strRxBuf.length; i++) {

                            sb.append(HexUtil.byteToHex(strRxBuf[i]) + ",");
                        }
                    }
                    ...
                    if (res.length() > 0) {
                        ...
                        if (receivedCallback != null) {
                            receivedCallback.callback(res.substring(0, res.length() - 1));
                        }
                    }
                    Log.i(TAG, "接收成功");
                }
